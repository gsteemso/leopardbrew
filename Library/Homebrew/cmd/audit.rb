...
class FormulaText
  ...
  def without_patch; @text.split("\n__END__").first; end

  def has_DATA?; /^[^#]*\bDATA\b/ =~ @text; end

  def has_END?; /^__END__$/ =~ @text; end

  def has_trailing_newline?; /\Z\n/ =~ @text; end

  def =~(regex); regex =~ @text; end
...
class FormulaAuditor
  ...
  end # FormulaAuditor#audit_class
  ...
  end # FormulaAuditor#audit_formula_name
  ...
  end # FormulaAuditor#audit_conflicts
    ...
    formula.options.each do |o|
      ...
      problem "Options should begin with with/without. Migrate “--#{o.name}” with “deprecated_option”." \
        if o.name !~ %r{with(out)?-} \
          and not ['32-bit', 'HEAD', 'c++11', 'cross', 'devel', 'local', 'native', 'universal'].any?{ |opt| o.name == opt }
    end
  ...
  def audit_desc
    ...
    if desc =~ /(([Cc])ommand ?line)/
      problem "Description should use “#{$2}ommand-line” instead of “#{$1}”"
    end

    if desc =~ /^([Aa]n?)\s/
      problem "Please remove the indefinite article “#{$1}” from the beginning of the description"
    end
  end # FormulaAuditor#audit_desc
  ...
  end # FormulaAuditor#audit_homepage
  ...
  end # FormulaAuditor#audit_github_repository
  ...
  end # FormulaAuditor#audit_legacy_patches
  ...
  end # FormulaAuditor#audit_patch
  ...
  def audit_text
    ...
    if text =~ /system ["']npm["'], ["']install["']/ && text !~ %r[opt_libexec}/npm/bin]
      ...
      problem "Please add “ENV.prepend_path 'PATH', \"#{need_npm}\"” to “def install”"
    end
  end # FormulaAuditor#audit_text
  ...
  def audit_line(line, lineno)
    ...
    if line =~ %r{# if this fails, try separate make/make install steps}
      problem 'Please remove default template comments'
    end
    ...
    if line =~ /(#\{\w+\s*\+\s*['"][^}]+\})/
      problem "Try not to concatenate strings within string interpolation:\n   #{$1}"
    end

    # Prefer formula path shortcuts in expressions
    if line =~ %r{\W(prefix\s*[+/]\s*['"](bin|include|libexec|lib|sbin|share|Frameworks)((/)|['"]))}
      problem "“#{$1}” should be “#{$2.downcase}#{$4}”"
    end
    ...
    if line =~ %r[((\#\{prefix\}/share/man/|\#\{share\}/man/|\#\{man\}/)(man[1-8]))]
    ...
    if line =~ /#\s*depends_(on|group)\s+\[?\s*(.+)\s*($|\]|\s)/
      problem "Commented-out dep #{$2}"
    end
    ...
    if line =~ /if\s+ARGV\.includes?\?\s+'--(HEAD|devel)'/
      problem "Use “if build.#{$1.downcase}?”"
    end
    ...
    if line =~ /build\.includes?\?[\s\(]+['"]\-\-(.*)['"]/
      problem "Reference '#{$1}' without dashes"
    end

    if line =~ /build\.includes?\?[\s\(]+['"]with(out)?-(.*)['"]/
    ...
    if line =~ /unless build\.with\?(\(|\s+)["']([^"']+)["']/
      problem "Use “if build.without? '#{$2}'” instead of “unless build.with? '#{$2}'”"
    end

    if line =~ /unless build\.without\?(\(|\s+)["']([^"']+)["']/
      problem "Use if build.with? '#{$2}' instead of unless build.without? '#{$2}'"
    end
    ...
    if line =~ /ARGV\.(includes?|build_32_bit|build_(bottle|devel|head|stable|universal))\?/
      right = ($2 ? $2 : $1) + '?'
    ...
    if line =~ /(assert [^!]+\.includes?\?)/
      problem "Use “assert_match” instead of “#{$1}”"
    end
    ...
    if @strict
      if line =~ /system(?: .*)? ((["'])[^"' ]*(?:\s+[^"']*)+["'])/
        ...
        unless %w[| < > & ; *].any? { |c| bad_system.include? c }
          good_system = bad_system.gsub(' ', "#{$2}, #{$2}")
  ...
  end # FormulaAuditor#audit_caveats
  ...
  end # FormulaAuditor#audit_reverse_migration
  ...
  def audit_prefix_has_contents
    ...
    EOS
  end # FormulaAuditor#audit_prefix_has_contents
  ...
  end # FormulaAuditor#audit_conditional_dep

  def quote_dep(dep); Symbol === dep ? dep.inspect : "'#{dep}'"; end

  def audit_check_output(output); problem(output) if output; end
  ...
  end # FormulaAuditor#audit

  private

  def problem(p); @problems << p; end

  def head_only?(formula); formula.head && formula.devel.nil? && formula.stable.nil?; end

  def devel_only?(formula); formula.devel && formula.stable.nil?; end
end # FormulaAuditor
...
class ResourceAuditor
  ...
  def initialize(resource)
    ...
    @problems = []
  end # ResourceAuditor#initialize
  ...
  end # ResourceAuditor#audit_checksum
  ...
  end # ResourceAuditor#audit_download_strategy
  ...
  end # ResourceAuditor#audit_urls

  def problem(text); @problems << text; end
end # ResourceAuditor
