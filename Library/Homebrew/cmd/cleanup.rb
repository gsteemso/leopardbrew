...
module Homebrew
  def cleanup
    ...
    else ARGV.resolved_formulae.each { |f| cleanup_formula(f) }; end
  end # cleanup
  ...
  def cleanup_cache
    ...
    HOMEBREW_CACHE.children.each do |path|
      if prune?(path)
        ...
        elsif path.directory? and path.to_s.include?('--') then cleanup_path(path) { FileUtils.rm_rf path }; end
        next
      end
  ...
  def cleanup_logs
    ...
    HOMEBREW_LOGS.subdirs.each{ |dir| cleanup_path(dir) { dir.rmtree } if prune?(dir, :days_default => 14) }
  end
