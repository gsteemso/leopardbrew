...
class Checks
  ...
  def check_for_macgpg2
    return if File.exists? "/usr/local/MacGPG2/share/gnupg/VERSION"
  ...
  def check_for_stray_dylibs
    ...
    __check_stray_files "/usr/local/lib", "*.dylib", white_list, <<-EOS.undent
      ...
      building Leopardbrew formulae, and may need to be deleted.

      Unexpected dylibs:
    EOS
  end # check_for_stray_dylibs
  ...
  def check_for_stray_static_libs
    ...
    EOS
  end # check_for_stray_static_libs
  ...
  def check_for_broken_symlinks
    ...
    Keg::PRUNEABLE_DIRECTORIES.each do |d|
      ...
      d.find do |path|
        if path.symlink? and not path.resolved_path_exists?
          broken_symlinks << path
        end
      end
    end # each pruneable directory |d|
  ...
  if MacOS.version >= "10.9"
    ...
    def check_xcode_up_to_date
      if MacOS::Xcode.installed? and MacOS::Xcode.outdated?
        <<-EOS.undent
    ...
    end # check_xcode_up_to_date, OS ≥ 10.9
    ...
    end # check_for_installed_developer_tools, OS 10.7 / 10.8

    def check_xcode_up_to_date
      if MacOS::Xcode.installed? and MacOS::Xcode.outdated? then <<-EOS.undent
      Your Xcode (#{MacOS::Xcode.version}) is outdated
      Please update to Xcode #{MacOS::Xcode.latest_version}.
    ...
    end # check_clt_up_to_date, OS 10.7 / 10.8
  else
    def check_for_installed_developer_tools
  ...
  def check_for_osx_gcc_installer
    if (MacOS.version < "10.7" or MacOS::Xcode.version > "4.1") and \
  ...
  end # check_for_osx_gcc_installer
  ...
  end # check_for_bad_install_name_tool
  ...
  def __check_subdir_access(base)
    ...
    EOS
  end # __check_subdir_access

  def check_access_share_locale; __check_subdir_access "share/locale"; end

  def check_access_share_man; __check_subdir_access "share/man"; end
  ...
  end # check_access_homebrew_repository
  ...
  def check_access_usr_local
    ...
    unless HOMEBREW_PREFIX.writable_real? then <<-EOS.undent
        ...
        Even if this directory was writable when you installed Leopardbrew, other
  ...
  (Keg::TOP_LEVEL_DIRECTORIES + ["lib/pkgconfig"]).each do |d|
    define_method("check_access_#{d.sub("/", "_")}") do
      ...
      if dir.exists? and not dir.writable_real? then <<-EOS.undent
          #{dir} isn't writable.
  ...
  end # define a bunch of directory‐testing methods

  def check_access_site_packages
    if Language::Python.homebrew_site_packages.exists? and not Language::Python.homebrew_site_packages.writable_real?
      <<-EOS.undent
  ...
  def check_access_cache
    if HOMEBREW_CACHE.exists? and not HOMEBREW_CACHE.writable_real?
      <<-EOS.undent
  ...
  end # check_user_path_3

  def check_for_bad_curl
    if MacOS.version <= "10.6" and not Formula["curl"].installed? then <<-EOS.undent
        The system curl on 10.6 and below is incapable of supporting modern
        secure connections & will fail on fetching pretty much anything.
        We recommend you:
            brew install curl
      EOS
    end
  end # check_for_bad_curl

  def check_user_curlrc
    if %w[CURL_HOME HOME].any? { |key| ENV[key] and File.exists?("#{ENV[key]}/.curlrc") }
      <<-EOS.undent
        You have a curlrc file
        If you have trouble downloading packages with Leopardbrew, then maybe this
        is the problem?  If the following command doesn’t work, then try removing
        your curlrc:
            curl https://github.com/
      EOS
    end
  end # check_user_curlrc

  def check_for_unsupported_curl_vars
    # Support for SSL_CERT_DIR seemed to be removed in the 10.10.5 update.
    if MacOS.version >= :yosemite and not ENV["SSL_CERT_DIR"].nil? then <<-EOS.undent
  ...
  end # check_for_unsupported_curl_vars
  ...
  end # check_which_pkg_config

  # despite prior maintainers’ inexplicable beliefs to the contrary, no version of Mac OS to which
  # I have access has included gettext and/or libintl.  Testing whether the gettext formula is
  # linked, which it now always is, is not useful; the check has been removed.

  def check_for_linked_iconv
    unless find_relative_paths("lib/libiconv.dylib", "include/iconv.h").empty?
      if (f = Formulary.factory("libiconv") rescue nil) and f.linked_keg.directory?
        <<-EOS.undent
          Your brewed libiconv is linked.  This will break stuff.  For serious.
          Unlink it with “brew unlink libiconv”.
        EOS
      else
        s = <<-EOS.undent_________________________________________________________72
          libiconv files detected at a system prefix other than /usr
          Leopardbrew’s libiconv formula is keg‐only; unless it’s explicitly reques‐
          ted, formulæ and third‐party software expect to link against the system
          version in /usr.  Libiconv in other prefixes can cause compile or link
          failure, especially if compiled with improper architectures.  OS X itself
          never installs anything to /usr/local, so it was installed by a user or by
  ...
  end # check_for_linked_iconv

  def check_for_config_scripts
    return unless HOMEBREW_CELLAR.exists?
    # HOMEBREW_CELLAR is set to $HOMEBREW_CELLAR.realpath when first imported from the environment.
    # Calling realpath on it again is not needed.
    ...
    inject_file_list scripts, <<-EOS.undent
      "config" scripts exist outside your system or Leopardbrew directories.
      `./configure` scripts often look for *-config scripts to determine if
      software packages are installed, and what additional flags to use when
      compiling and linking.

      Having additional scripts in your path can confuse software installed via
      Leopardbrew if the config script overrides a system or Leopardbrew provided
      script of the same name. We found the following "config" scripts:
    EOS
  end # check_for_config_scripts
  ...
  def check_for_multiple_volumes
    ...
    unless where_cellar == where_temp then <<-EOS.undent
        Your Cellar and TEMP directories are on different volumes.  Mac OS won’t move
        relative symlinks across volumes unless the target file already exists.  Brews
        known to be affected by this are Git and Narwhal.

        You should set the $HOMEBREW_TEMP environment variable to a suitable directory
        on the same volume as your Cellar.
      EOS
    end
  end # check_for_multiple_volumes
  ...
  def check_for_git
    if Utils.git_available?
      __check_git_version
        ...
        Leopardbrew uses Git for several internal functions, and some formulae use Git
  ...
  end # check_git_newline_settings

  def check_git_origin
    return if not (Utils.git_available? and (HOMEBREW_REPOSITORY/".git").exists?)
    ...
    if origin.nil? then <<-EOS.undent
      ...
      EOS
        ...
        origin remote to point at the main repository, located at:
            https://github.com/gsteemso/leopardbrew.git
      EOS
    end
  end # check_git_origin
  ...
  def check_for_autoconf
    ...
    unless autoconf.nil? or safe_autoconfs.include?(autoconf.to_s) then <<-EOS.undent
        ...
        This custom autoconf may cause some Leopardbrew formulæ to fail to compile.
      EOS
    end
  end # check_for_autoconf

  def __check_linked_brew(f)
    f.rack.subdirs.each do |iprefix|
      iprefix.find do |src|
        next if src == iprefix
        dst = HOMEBREW_PREFIX + src.relative_path_from(iprefix)
        return true if dst.symlink? and src == dst.resolved_path
      end
    end

    false
  end # __check_linked_brew

  def check_for_linked_keg_only_brews
    return unless HOMEBREW_CELLAR.exists?

    linked = Formula.installed.select do |f|
      f.keg_only? and __check_linked_brew(f)
    end
    ...
    HOMEBREW_REPOSITORY.cd do
      unless `git status --untracked-files=all --porcelain -- Library/Homebrew/ 2>/dev/null`.chomp.empty?
        <<-EOS.undent_________________________________________________________72
          You have uncommitted modifications to Leopardbrew
          If this a surprise to you, then you should stash these modifications.
          Stashing returns Leopardbrew to a pristine state but can be undone
          should you later need to do so for some reason.
              cd #{HOMEBREW_LIBRARY} && git stash && git clean -d -f
        EOS
      end
    end
  end # check_git_status

  def check_for_enthought_python
    <<-EOS.undent if which "enpkg"
      Enthought Python was found in your PATH.
      This can cause build problems, as this software installs its own
      copies of iconv and libxml2 into directories that are picked up by
      other build systems.
    EOS
  end # check_for_enthought_python

  def check_for_library_python
    <<-EOS.undent if File.exists?("/Library/Frameworks/Python.framework")
  ...
  def check_for_non_prefixed_coreutils
    ...
    EOS
  end # check_for_non_prefixed_coreutils

  def check_for_non_prefixed_findutils
    default_names = Tab.for_name("findutils").with? "default-names"
    <<-EOS.undent if default_names
  ...
  def check_for_outdated_leopardbrew
    return unless Utils.git_available?
    HOMEBREW_REPOSITORY.cd do
      if File.directory? ".git"
        ...
        if remote.nil? or local == remote[0]
          return
        end
      end
      ...
      <<-EOS.undent if Time.now.to_i - timestamp > HOMEBREW_OUTDATED_LIMIT
        Your Leopardbrew may be outdated
        You haven’t updated Leopardbrew in a while.  To do so, run `brew update`.
      EOS
    end
  end # check_for_outdated_leopardbrew

  def check_for_unlinked_but_not_keg_only
    unlinked = Formula.racks.reject do |rack|
      unless (HOMEBREW_REPOSITORY/"Library/LinkedKegs"/rack.basename).directory?
        begin
  ...
  end # check_xcode_license_approved
  ...
  def check_for_pth_support
    ...
    EOS
  end # check_for_pth_support

  def check_for_external_cmd_name_conflict
    cmds = paths.flat_map { |p| Dir["#{p}/brew-*"] }.uniq
    cmds = cmds.select { |cmd| File.file?(cmd) and File.executable?(cmd) }
    cmd_map = {}
    cmds.each do |cmd|
    ...
    s
  end # check_for_external_cmd_name_conflict

  def all; methods.map(&:to_s).grep(/^check_/); end
