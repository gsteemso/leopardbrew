class Array
  ...
  def list
    _self = map{ |item| Symbol === item ? item.to__s : item.to_s }
    case length
      when 0 then ''
      when 1 then _self[0]
      when 2 then "#{_self[0]} and #{_self[1]}"
      else "#{_self[0..-2] * ', '}, and #{_self[-1]}"
    end
  end # Array#list
...
module Enumerable; alias_method :includes?, :include? unless method_defined? :includes?; end
...
class Symbol; def to__s; ":#{to_s}"; end; end
