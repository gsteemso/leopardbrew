...
module Stdenv
  ...
  def self.extended(base)
    unless ORIGINAL_PATHS.include? HOMEBREW_PREFIX/'bin' then base.prepend_path 'PATH', "#{HOMEBREW_PREFIX}/bin"; end
  end
  ...
  def setup_build_environment(archset = nil)
    ...
    if MacOS.version >= :mountain_lion then delete('LC_ALL'); self['LC_CTYPE']='C'; end
    # Set the default pkg-config search path, overriding the built-in paths.  (PKG_CONFIG_PATH is searched before these.)
    self['PKG_CONFIG_LIBDIR'] = determine_pkg_config_libdir
    # Make any aclocal stuff installed in Homebrew available.
    self['ACLOCAL_PATH'] = "#{HOMEBREW_PREFIX}/share/aclocal" if MacOS.has_apple_developer_tools? and
                                                                 MacOS::Xcode.provides_autotools?
    unless HOMEBREW_PREFIX.to_s == '/usr/local'  # This is already an -isystem and -L directory so we skip it.
      self['CPPFLAGS'] = "-isystem#{HOMEBREW_PREFIX}/include"
      self['LDFLAGS'] = "-L#{HOMEBREW_PREFIX}/lib"
      self['CMAKE_PREFIX_PATH'] = HOMEBREW_PREFIX.to_s  # CMake ignores CPPFLAGS and LDFLAGS.
    end
    ...
    set_cflags "-Os #{SAFE_CFLAGS_FLAGS}"  # Os is the default Apple uses for all its stuff so we’ll trust them.
    append 'LDFLAGS', '-Wl,-headerpad_max_install_names'
    send(compiler)
    if cc =~ GNU_GCC_REGEXP then append_path 'PATH', gcc_version_formula($&).opt_bin.to_s; end
    macosxsdk  # Add lib, include, etc. from the current macosxsdk to compiler flags.
    append_path 'PATH', "#{MacOS::Xcode.prefix}/usr/bin:#{MacOS::Xcode.toolchain_path}/usr/bin" if MacOS::Xcode.without_clt?
    ...
    if MacOS.version < :snow_leopard and Target.prefer_64b? and build_archs.detect{ |a| a.to_s.ends_with? '64' }
      ...
      # Many, many builds are broken by a buggy old stock ld.  Our ld64 fixes a lot of them, though obviously it can’t build itself.
  ...
  %w[fast O4 Og].each{ |opt| define_method(opt) {} }  # These methods are compatibility no‐ops.
  ...
  def clang
    ...
    map = map.merge(:nehalem => '-march=native -Xclang -target-feature -Xclang -aes')  # Clang mistakenly enables AES-NI on Nehalem
  ...
  def macosxsdk(version = MacOS.version)
    ...
    if (sdk = MacOS.sdk_path(version)) and not MacOS::CLT.installed?
      ...
      self['SDKROOT'] = sdk
      append_path 'CPATH', "#{sdk}/usr/include"  # Tell clang/gcc where the system includes are.
      append_to_cflags "-isysroot #{sdk}"        # The -isysroot is needed, too, because of the Frameworks.
      append 'CPPFLAGS', "-isysroot #{sdk}"
      append 'LDFLAGS', "-isysroot #{sdk}"       # The linker needs to find sdk/usr/lib/.
  ...
  def remove_macosxsdk(version = MacOS.version)
    ...
    if (sdk = MacOS.sdk_path(version)) and not MacOS::CLT.installed?
      ...
      remove ['CPPFLAGS', 'LDFLAGS'], "-isysroot #{sdk}"
      if HOMEBREW_PREFIX.to_s == '/usr/local' then delete('CMAKE_PREFIX_PATH')
      else self['CMAKE_PREFIX_PATH'] = HOMEBREW_PREFIX.to_s  # setup_build_environment sets it, so we have to restore it here.
      end
      remove 'CMAKE_FRAMEWORK_PATH', "#{sdk}/System/Library/Frameworks"
    end
  end # remove_macosxsdk

  def minimal_optimization; set_cflags "-Os #{SAFE_CFLAGS_FLAGS}"; macosxsdk unless MacOS::CLT.installed?; end

  def no_optimization; set_cflags SAFE_CFLAGS_FLAGS; macosxsdk unless MacOS::CLT.installed?; end

  # Some configure scripts won't find libxml2 without help
  def libxml2; append 'CPPFLAGS', "-I#{MacOS::CLT.installed? ? MacOS.sdk_path : ''}/usr/include/libxml2"; end
  ...
  def x11
    ...
    append_path 'CMAKE_PREFIX_PATH', "#{MacOS.sdk_path}/usr/X11" if MacOS::XQuartz.provided_by_apple? and not MacOS::CLT.installed?
  ...
  # @private
  def replace_in_cflags(before, after); CC_FLAG_VARS.each { |key| self[key] = self[key].sub(before, after) if key?(key) }; end
  ...
  def set_cpu_flags(flags, default = DEFAULT_FLAGS, map = Target.model_optflag_map)
    ...
    remove flags, '-Qunused-arguments'  # not really a 'CPU' cflag, but is only used with clang
  end # set_cpu_flags

  # @private
  def set_cpu_cflags(default = DEFAULT_FLAGS, map = Target.model_optflag_map); set_cpu_flags CC_FLAG_VARS, default, map; end
