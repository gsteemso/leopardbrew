    ...
    def model
      @@model ||= \
        case sysctl_int('hw.cpufamily')
          when 0x07d34b9f then :a12         # arm      0. [Cyprus/]Aruba (transition Minis; [A12X/]A12Z:  Vortex & Tempest cores)
#         when 0x0cc90e64 then :arm13       # arm      —  (?)
          when 0x0f817246 then :kabylake    # intel   11. Kaby Lake
          when 0x10b282dc then :haswell     # intel    8. Haswell
          when 0x17d5b93a,                  # arm      7. Brava xxx (M4 Pro/Max)
               0x6f5129ac then :m4          # arm      6. Donan (base M4)
...
#         when 0x53b005f5 then :xscale      # arm      —  (?)
          ...
          when 0x5f4dea93,                  # arm      4. Lobos (M3 Pro:  Everest & Sawtooth cores)
               0x72015832,                  # arm      5. Palma (M3 Max:  Everest & Sawtooth cores)
               0xfa33415e then :m3          # arm      3. Ibiza (base M3:  Everest & Sawtooth cores)
#         when 0x67ceee93 then :a10         # arm      —  Cayman/Myst/Gibraltar (A10[X]/T2:  Hurricane & Zephyr cores)
          ...
          when 0x73d67300,                  # intel    1. Yonah et al:  Core Solo/Duo  (T1200, T2x00, L2400)
               0xaa33392b then :core        # intel    0. ???? (developer‐transition model)
...
#         when 0x8ff620d8 then :arm11       # arm      —  (?)
#         when 0x92fb37c8 then :a9          # arm      —  Maui/Malta/Elba:  A9[X] (Twister core)
#         when 0x96077ef1 then :arm14       # arm      —  (?)
#         when 0xa8511bca then :arm15       # arm      —  (?)
#         when 0xbd1b0ae9 then :arm12       # arm      —  (?)
...
#         when 0xe73283ae then :arm09       # arm      —  (?)
#         when 0xe81e7ef6 then :a11         # arm      —  Skye (A11:  Monsoon & Mistral cores)
  ...
  class << self
    ...
    ARCH_DATA = {
      :ppc     => {:bits => 32, :type => :powerpc, :oldest => (Sysctl.model == :g3 ? :g3 : :g4)},  # Altivec, unless we _can’t_.
    ...
    # What flags do you use when the target CPU is newer than your compiler knows about?

    # TODO: Fill this out alongside the Clang optflags
    def clang_flags_for_post_(v, t = type); ''; end

    def gcc_flags_for_post_(v, t = type)
      case t
        when :arm then raise ArgumentError, 'Our GCC versions are all too old to know about the arm64 architecture yet.'
        when :intel
          v < 4.2 ? '-march=nocona -mssse3' : \
          v < 4.9 ? '-march=core2 -msse4.2' : \
          v <  5  ? '-march=broadwell'      : \
          v <  6  ? '-march=broadwell -mclflushopt -mxsavec -mxsaves' : \
          v <  8  ? '-march=skylake-avx512 -mavx512ifma -mavx512vbmi -msha' : ''
#       when :powerpc then ''  # All PowerPC CPUs are known to all our compilers.
        else ''
      end # case CPU type
    ...
    def known_archs; ARCH_DATA.keys.extend ALE; end

    def known_models; MODEL_DATA.keys; end

    def model_is_known?(m); MODEL_DATA.has_key?(m); end

    def archs(t = type); (type_data(t)[:archs]).dup.extend ALE; end
    alias :archs_of_type :archs

    def which_clang_knows_about(m = model); if model_is_known?(m) then model_data(m)[:clang][:vrsn]; end; end

    def which_gcc_knows_about(m = model); if model_is_known?(m) then model_data(m)[:gcc][:vrsn]; end; end
