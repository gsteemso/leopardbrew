...
module Merge
  ...
  def merge_c_headers(archs, sub_path = nil)
    ...
    # dependent headers in question are present on all architectures.
    s_p = (sub_path ? sub_path + '/' : '')  # Don’t suffer a double slash when sub_path is null.
    Dir["#{stashdir(:header, archs[0])}/#{s_p}*"].each do |basis_file|
      ...
      if File.directory?(basis_file) then merge_c_headers(archs, spb)
        ...
        diffpoints = {}  # Keyed by line number in the basis file.  Each value’s an array of three‐element hashes, 1 hash per arch;
                         # each such hash contains the arch, the hunk’s displacement (number of basis‐file lines it replaces), & an
                         # array of its lines.
        archs[1..-1].each do |a|
          raw_diffs = `diff --minimal --unified=0 #{basis_file} #{stashdir(:header, a)}/#{spb}`
          next unless raw_diffs.choke
          # The unified diff output begins with two lines identifying the source files, followed by a series of chunk records, each
          # describing one difference that was found.  Each chunk record begins with a line that looks like:
          ...
          diff_chunks.each do |d|
            ...
            unless diffpoints.has_key?(base_linenumber_string) then diffpoints[base_linenumber_string] = []; end
            # If the chunk length is 1, the comma and second number are not present.
            length_match = d.match(%r{\A@@ -\d+(?:,(\d+))?})[1].nope || 1
            line_group = []
            # We want the lines that are either unchanged between files, or only found in the non‐basis file; & we’ll shave off the
            # leading ‘+’ or ‘ ’.
        ...
        # TODO:  Ideally, overlapping and/or different-displacement chunks would be accommodated at this point; but as few packages
        # seem to generate those in the first place, it can wait.  However, packages do exist (e.g., the originals of both Python 2
        # and Python 3) that can & will generate quad fat binaries, so it can’t be ignored forever.  This also doesn’t test whether
        # it might interfere with existing conditional-compilation blocks, because what to do if it _would_ is far from obvious.
        basis_lines = []
        ...
        diffpoints.keys.sort.reverse.each do |index_string|
          ...
          adjusted_lines = [
              ...
              *(diffpoints[index_string].map{ |dp| [ "\#elif defined (__#{dp[:arch]}__)\n", *(dp[:chunk_lines]) ] }),
              "\#endif\n"
            ]
